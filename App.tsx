  ...
  return (
    <div className="max-w-6xl mx-auto py-12 px-4">
      <div className="text-center mb-16">
        <h1 className="text-6xl font-black text-math-800 mb-6 tracking-tighter italic">GéoDémon</h1>
      ...
      {completed.length > 0 && (
        <section className="mb-16 animate-in fade-in slide-in-from-left-4 duration-700">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link to="/progression" className="text-math-600 font-bold hover:underline flex items-center gap-2 text-sm uppercase tracking-widest">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        {grades.map((grade) => (
          <Link
            key={grade}
            to={`/grade/${grade}`}
            className="group p-10 bg-white rounded-3xl shadow-xl border-4 border-transparent hover:border-math-500 transition-all transform hover:-translate-y-2 relative overflow-hidden"
          >
            ...
            <div className="relative z-10 flex items-center justify-between">
              <div>
                <span className="text-5xl font-black text-math-600 tracking-tighter">{grade}</span>
                <h3 className="text-xl font-bold text-gray-800 mt-2">Niveau Collège</h3>
                <p className="text-gray-500 mt-2 font-medium">Explorer les théorèmes et démonstrations de {grade}.</p>
              </div>
              <div className="w-16 h-16 bg-math-100 rounded-2xl flex items-center justify-center text-math-600 group-hover:bg-math-600 group-hover:text-white transition-all shadow-inner">
...
const ProgressionPage: React.FC = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto py-12 px-4">
      <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-12 bg-white p-10 rounded-3xl border border-gray-100 shadow-xl">
        <div className="flex items-center gap-8">
          <div className={`w-32 h-32 ${rank.bg} rounded-full flex items-center justify-center text-6xl shadow-inner border-4 border-white`}>
            <i className={`fas ${rank.icon} ${rank.color}`}></i>
          </div>
          <div>
            <h2 className="text-4xl font-black text-gray-900 mb-1">Tableau de Bord</h2>
            <p className={`font-black uppercase tracking-[0.2em] text-sm ${rank.color}`}>{rank.label}</p>
          </div>
        </div>
        <div className="flex gap-6">
           <div className="text-center">
             <p className="text-xs font-black text-gray-400 uppercase mb-1">Score Total</p>
             <p className="text-4xl font-black text-math-600">{MOCK_PROGRESS.points} <span className="text-lg">pts</span></p>
           </div>
           ...
           <div className="text-center">
             ...
             <p className="text-4xl font-black text-logic-600">{completed.length}</p>
           </div>
        </div>
      </div>
      ...
      <div className="mb-16">
        <div className="flex justify-between mb-4 items-end">
          <h3 className="text-xl font-bold text-gray-800">Maîtrise Globale</h3>
          <span className="text-2xl font-black text-math-600">{progressPercent}%</span>
        </div>
        <div className="w-full bg-gray-100 rounded-full h-6 p-1 overflow-hidden shadow-inner border border-gray-200">
          <div 
      ...
      <section>
        <div className="flex items-center justify-between mb-8 border-b pb-4">
          <h3 className="text-3xl font-black text-gray-900 tracking-tight">Exercices Terminés</h3>
          <span className="bg-logic-100 text-logic-700 px-4 py-1 rounded-full text-sm font-bold">{completed.length} succès</span>
        </div>
        
        <div className="grid gap-4">
          {completed.length > 0 ? (
            completed.map((ex) => (
              <div key={ex.id} className="bg-white p-6 rounded-2xl border border-gray-100 flex items-center justify-between shadow-sm hover:shadow-md transition-all group">
                <div className="flex items-center gap-6">
                  <div className="w-14 h-14 bg-logic-50 text-logic-600 rounded-2xl flex items-center justify-center text-xl shadow-inner group-hover:bg-logic-500 group-hover:text-white transition-colors">
                    <i className="fas fa-check"></i>
                  </div>
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="px-2 py-0.5 bg-math-100 text-math-700 rounded text-[10px] font-bold uppercase">{ex.grade}</span>
                      <span className="text-gray-400 text-xs font-bold uppercase tracking-wider">{ex.theme}</span>
                    </div>
                    <h4 className="text-xl font-bold text-gray-800">{ex.title}</h4>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <Link 
                    to={`/exercise/${ex.id}`} 
                    className="px-6 py-3 bg-gray-50 text-math-600 rounded-xl hover:bg-math-600 hover:text-white transition-all font-bold text-sm shadow-sm"
                  >
            ...
            <div className="bg-gray-50 p-20 rounded-3xl text-center border-2 border-dashed border-gray-200">
              <div className="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center text-gray-300 mx-auto mb-6 text-3xl">
                <i className="fas fa-tasks"></i>
              </div>
              <h4 className="text-xl font-bold text-gray-400">Aucun exercice terminé pour l'instant</h4>
              <p className="text-gray-400 mt-2">Choisis ton niveau sur l'accueil et commence à pratiquer !</p>
              <Link to="/" className="mt-8 inline-block px-8 py-3 bg-math-600 text-white rounded-xl font-bold hover:bg-math-700 transition-colors">
                Voir les exercices
              </Link>
            </div>
          )}
...
const GradePage: React.FC<{ grade: string }> = ({ grade }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
      ...
      <div className="bg-white p-12 rounded-[3rem] shadow-xl border border-gray-100 mb-12 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-math-50 rounded-full -mr-20 -mt-20 opacity-30"></div>
        <div className="relative z-10">
           <h2 className="text-5xl font-black text-gray-900 mb-4 tracking-tighter">Parcours {grade}</h2>
           <p className="text-lg text-gray-500 font-medium">Sélectionne un défi pour tester ta rigueur mathématique.</p>
        </div>
      </div>
          ...
          {filtered.map((ex) => {
            ...
            return (
              <Link
                key={ex.id}
                to={`/exercise/${ex.id}`}
                className={`group block p-8 bg-white rounded-3xl shadow-sm border-2 transition-all hover:shadow-xl hover:-translate-y-1 ${isCompleted ? 'border-logic-200 bg-logic-50/10' : 'border-transparent'}`}
              >
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-6">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl transition-all ${isCompleted ? 'bg-logic-500 text-white' : 'bg-gray-100 text-gray-400 group-hover:bg-math-500 group-hover:text-white'}`}>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        <span className="text-gray-400 text-sm font-bold uppercase tracking-wider">{ex.theme}</span>
                  ...
                  <div className="flex items-center gap-4">
                     {isCompleted && <span className="text-logic-600 font-black text-xs uppercase bg-logic-100 px-3 py-1 rounded-full">Réussi</span>}
                     <div className="w-10 h-10 bg-gray-50 rounded-full flex items-center justify-center text-gray-300 group-hover:bg-math-100 group-hover:text-math-600 transition-all">
...
const ExercisePage: React.FC<{ id: string }> = ({ id }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4">
      <button onClick={() => navigate(-1)} className="text-math-600 hover:underline mb-8 flex items-center font-bold uppercase tracking-widest text-sm">
        <i className="fas fa-arrow-left mr-2"></i> Quitter l'exercice
      </button>
      ...
      <div className="grid lg:grid-cols-2 gap-12">
        ...
        <div className="space-y-8">
          <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-gray-100 relative overflow-hidden">
            <div className="absolute top-0 right-0 p-8 text-math-50 text-8xl opacity-20 -mr-4 -mt-4">
              <i className="fas fa-shapes"></i>
            </div>
            <div className="relative z-10">
              <h2 className="text-3xl font-black text-gray-900 mb-6">{exercise.title}</h2>
              <div className="text-xl leading-relaxed text-gray-700 bg-math-50/50 p-8 rounded-3xl border-2 border-dashed border-math-200">
                <i className="fas fa-quote-left text-math-300 mb-4 block text-2xl"></i>
                {exercise.statement}
              </div>
            </div>
          </div>
          ...
          {exercise.geogebraId && (
            <div className="shadow-2xl rounded-3xl overflow-hidden border-8 border-white bg-white">
          ...
          <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100">
            ...
            <div className="space-y-4">
              {PROPERTIES.filter(p => p.id === exercise.solution.propertyId).map(p => (
                <div key={p.id} className="p-6 bg-math-50 rounded-2xl border-l-8 border-math-400">
                  <p className="text-xs font-black text-math-700 uppercase mb-2 tracking-widest">{p.name}</p>
                  <p className="text-lg italic text-gray-700 leading-snug">"{p.statement}"</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Right: Redaction & AI */}
        <div className="space-y-8">
          <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-gray-100 ring-1 ring-gray-200">
            <div className="flex justify-between items-center mb-10">
              <h3 className="text-3xl font-black text-gray-900 flex items-center">
                <i className="fas fa-pen-nib text-math-600 mr-4"></i> Rédaction
              </h3>
            ...
            <button
              ...
              className="mt-10 w-full py-5 bg-gradient-to-r from-math-600 to-math-800 text-white font-black text-xl rounded-2xl hover:scale-[1.02] active:scale-[0.98] transition-all shadow-xl disabled:opacity-50 disabled:hover:scale-100 flex items-center justify-center gap-4"
            >
              {loading ? (
                <><i className="fas fa-circle-notch fa-spin"></i> Analyse Professeur...</>
              ) : (
                <><i className="fas fa-brain"></i> Soumettre au tuteur IA</>
              )}
            </button>
          </div>

          {feedback && (
            <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl border-4 border-math-100 animate-in zoom-in-95 duration-500 relative overflow-hidden">
              <div className="absolute top-0 left-0 w-2 h-full bg-math-600"></div>
              <div className="flex items-center gap-4 mb-6">
                <div className="w-14 h-14 bg-math-600 rounded-2xl flex items-center justify-center text-white text-2xl shadow-lg shadow-math-200">
                  <i className="fas fa-user-graduate"></i>
                </div>
                <div>
                  <h3 className="text-2xl font-black text-math-800 tracking-tight">Conseils du Tuteur</h3>
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Feedback instantané</p>
                </div>
              </div>
              <div className="text-lg text-gray-700 leading-relaxed bg-math-50/80 p-8 rounded-3xl border border-math-100 italic font-medium whitespace-pre-wrap">
                {feedback}
              </div>
            </div>
          )}
  ...
  return (
    <Router>
      <div className="min-h-screen bg-gray-50 flex flex-col font-sans selection:bg-math-200 selection:text-math-900">
        <nav className="bg-white/80 backdrop-blur-md shadow-sm border-b px-8 py-5 flex justify-between items-center sticky top-0 z-50">
          <Link to="/" className="flex items-center gap-4 group">
            <div className="w-12 h-12 bg-gradient-to-br from-math-600 to-math-800 rounded-2xl flex items-center justify-center text-white text-2xl font-black group-hover:scale-110 transition-transform shadow-lg shadow-math-200">
              GΔ
            </div>
            <div>
              <span className="text-2xl font-black text-math-800 tracking-tighter block leading-none">GéoDémon</span>
              <span className="text-[10px] font-black text-math-500 uppercase tracking-[0.3em]">Collège Elite</span>
            </div>
          </Link>
        ...
        <footer className="bg-white border-t py-16 px-8 mt-20">
          <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-12 text-center md:text-left">
            <div>
              <div className="flex items-center gap-3 mb-6 justify-center md:justify-start">
                 ...
                 <span className="text-xl font-black text-math-800 tracking-tight">GéoDémon</span>
              </div>
              <p className="text-gray-400 font-medium leading-relaxed">L'excellence mathématique accessible à tous, guidée par une intelligence artificielle bienveillante.</p>
            </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <a href="#" className="text-gray-400 hover:text-math-600 font-bold transition-colors">Forum d'entraide</a>
              <a href="#" className="text-gray-400 hover:text-math-600 font-bold transition-colors">Guide Enseignants</a>
          ...
          <div className="max-w-6xl mx-auto mt-16 pt-8 border-t text-center">
            <p className="text-gray-300 text-xs font-bold uppercase tracking-widest">© 2025 GéoDémon Collège • Conçu pour la réussite scolaire</p>
          </div>
        </footer>
      </div>
    </Router>
